        ...
        if G::IS_BROWSER {
            spawn_local_scoped(context, async move {
                ...
                match query_response {
                    Some(response) => {
                        let server_challenge: SecurityKeyChallenge =
                            serde_json::from_str(&response).unwrap();
                        let challenge: Object =
                            Uint8Array::from(server_challenge.challenge.as_slice())
                                .deref()
                                .to_owned();
                        let user_id: Uint8Array =
                            Uint8Array::from(server_challenge.user_id.to_string().as_bytes());

                        let public_key_credential_parameters: PublicKeyCredentialParameters =
                            PublicKeyCredentialParameters::new(
                                SHA_256_SIGNATURE_ALGORITHM,
                                web_sys::PublicKeyCredentialType::PublicKey,
                            );

                        let mut relaying_party: PublicKeyCredentialRpEntity =
                            PublicKeyCredentialRpEntity::new(
                                server_challenge.relaying_party.as_str(),
                            );
                        relaying_party.id(&server_challenge.relaying_party_id);

                        let public_key_credential_parameters_array: Array =
                            Array::new_with_length(1);
                        public_key_credential_parameters_array
                            .set(0, JsValue::from(public_key_credential_parameters));

                        let user: PublicKeyCredentialUserEntity =
                            PublicKeyCredentialUserEntity::new(
                                email_address.get().as_ref(),
                                USER_DISPLAY_NAME,
                                &user_id.deref(),
                            );
                        ...
                        public_key_credential_creation_options
                            .attestation(web_sys::AttestationConveyancePreference::Direct);

                        let mut authentication_selection_criteria: AuthenticatorSelectionCriteria =
                            AuthenticatorSelectionCriteria::new();
                        authentication_selection_criteria.authenticator_attachment(
                            web_sys::AuthenticatorAttachment::CrossPlatform,
                        );
                        public_key_credential_creation_options
                            .authenticator_selection(&authentication_selection_criteria);

                        let mut credential_creation_options: CredentialCreationOptions =
                        ...
                        let credentials_result = JsFuture::from(
                            Window::navigator(&window().unwrap())
                                .credentials()
